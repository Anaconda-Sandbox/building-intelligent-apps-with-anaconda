...
# wasp18b_lightcurve.csv search order:
#   1. module root (committed directly to 06-app-architecture/)
#   2. 01-data-sources/ (standard repo location)
#   3. polars_demo submodule fallback
LOCAL_CSV="$MODULE_DIR/wasp18b_lightcurve.csv"
...
if [[ -f "$LOCAL_CSV" ]]; then
  ROWS=$(wc -l < "$LOCAL_CSV")
  ok "wasp18b_lightcurve.csv (local) — $((ROWS - 1)) rows"
elif [[ -f "$DATA_PATH" ]]; then
  ROWS=$(wc -l < "$DATA_PATH")
  ok "wasp18b_lightcurve.csv — $((ROWS - 1)) rows"
elif [[ -d "$POLARS_DEMO_DIR" && -z "$(ls -A "$POLARS_DEMO_DIR" 2>/dev/null)" ]]; then
  ...
  warn "CSV found in polars_demo — copying to module root"
  cp "$FALLBACK_PATH" "$LOCAL_CSV"
  ok "wasp18b_lightcurve.csv copied to module root"
else
  err "wasp18b_lightcurve.csv not found"
  echo ""
  echo "     Expected at: $LOCAL_CSV"
...
# Guard: verify duckdb is importable before attempting the demo
if ! "$PYTHON" -c "import duckdb" 2>/dev/null; then
  err "duckdb not installed in conda env '$EXPECTED_ENV'"
  echo ""
  echo "     Fix:"
  echo "       conda install -n $EXPECTED_ENV conda-forge::duckdb"
